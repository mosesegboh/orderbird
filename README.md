...
Hey Front End developer. Welcome. Your mission, should you decide to accept it,
is to carve out 3 hours and create a SPA using [Laravel][1], [Tailwind][2], and
your JavaScript library of preference.

[1]: https://laravel.com/
[2]: https://tailwindcss.com/

Please read these instructions in detail, specially the *"Required steps for a
valid solution"*, as you'll be evaluated based on these points.
...
horizontally on desktop and tablet, with a max. width of `768px`, but occupy the
entire screen on mobile devices. Allow horizontal scroll to prevent breaking the
rows only when necessary.
...
Right after `docker compose up`, the container `laravel` will error until you
run `composer install` from inside of the container. This is part of the
...
4. Write a migration to create a users table with: first name, last name and
   email address. The migration should be able to rollback
5. Write a seed to create 20 users
6. Write a view to display the users data from the database in a table. Fetch
   the data from the `/users` endpoint
7. Write a JavaScript solution to sort the users table by any of the fields:
   you can use any library (React, Vue, Svelte, etc.), but you **can't implement** a
   pre-made package with this functionality (e.g. [DataTables][4])
...
- The quality of your code; keep it tidy, linted, styled, and DRY:
...
- Opening a modal to show the details of the user when their name is clicked
- Anything else you would like to add

## When you finish

Please send your solution as a zipped/tarred package to
